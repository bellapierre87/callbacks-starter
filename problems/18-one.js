...
let one = function(arr, cb) {
    let counter = 0;  //set the counter

    //for each element and index in array, pass it in the cb function
    //if result is true, increment the counter by 1
    arr.forEach(function(el, i){
        if(cb(el, i)){
            counter++;
        }
    })
    //if counter = 1, then return true
    //else return false;
    if(counter ===1 ){
        return true
    }
