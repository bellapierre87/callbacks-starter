...
let chainMap = function(num, ...cb) {
    ...
    let result = num; // set result to be the initial value

    //iterate through the callbacks
   for (let i in cb){

    //use result of the prev callback function as the parameter of the next until done
    //once done with all, return final result
    result = cb[i](result);
   }
     return result;
