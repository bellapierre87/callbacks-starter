...
let xorSelect = function(arr, cb1, cb2) {
  ...
  //for each, see if cb1 is true but not cb2 when passing element,
  //or cb2 is true but not cb1 when passing element
  //if so then add to new array
  arr.forEach(function(el){
      if((cb1(el) === true && cb2(el)=== false)  || (cb2(el) === true && cb1(el)=== false)){
        newArr.push(el);
