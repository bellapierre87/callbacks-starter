...
let sentenceMapper = function(sentence, cb) {
    let arr = sentence.split(" "); //convert string sentence to an array of strings

    let newArr = [];

    // for each element, pass it in cb and add result to newArr
    arr.forEach(function(el){
        newArr.push(cb(el));

    });
    let newSentence = newArr.join(" "); // convert newArr to a new sentense

    return newSentence;  //return new sentence
